{% set columns = adapter.get_columns_in_relation(ref('serving_all_data_cytotox')) %}
{% set total_columns = columns | length %}

with base as (
    select * from {{ ref('serving_all_data_cytotox') }}
),
row_completeness as (
    select
        *,
...
select
    ...
    count(distinct nanoparticle || '_' || cell_line_id) as unique_np_cell_line_pairs,
